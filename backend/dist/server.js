...
/*
  ...
  Retrieves detected anomalies and unusual patterns in performance data
*/
app.get('/api/workspaces/:workspace_id/anomalies', authenticateToken, validateWorkspaceAccess, async (req, res) => {
    try {
        ...
        const date_to = getQueryParam(req.query.date_to);
        const platform = getQueryParam(req.query.platform);
        const severity = getQueryParam(req.query.severity, 'medium');
        const limit = getQueryParamAsNumber(req.query.limit, 50);
        const offset = getQueryParamAsNumber(req.query.offset, 0);
        ...
        try {
            ...
            // Query to detect anomalies based on performance deviations
            const anomalyQuery = `
                WITH daily_metrics AS (
                    SELECT 
                        ...
                        m.conversions,
                        CASE 
                            WHEN SUM(m.spend) OVER (PARTITION BY m.platform ORDER BY m.date ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING) > 0
                            THEN (m.spend - AVG(m.spend) OVER (PARTITION BY m.platform ORDER BY m.date ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING)) 
                                 / STDDEV(m.spend) OVER (PARTITION BY m.platform ORDER BY m.date ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING)
                            ELSE 0 
                        END as spend_zscore,
                        CASE 
                            WHEN SUM(m.revenue) OVER (PARTITION BY m.platform ORDER BY m.date ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING) > 0
                            THEN (m.revenue - AVG(m.revenue) OVER (PARTITION BY m.platform ORDER BY m.date ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING)) 
                                 / STDDEV(m.revenue) OVER (PARTITION BY m.platform ORDER BY m.date ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING)
                            ELSE 0 
                        END as revenue_zscore,
                        CASE 
                            WHEN SUM(m.conversions) OVER (PARTITION BY m.platform ORDER BY m.date ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING) > 0
                            THEN (m.conversions - AVG(m.conversions) OVER (PARTITION BY m.platform ORDER BY m.date ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING)) 
                                 / STDDEV(m.conversions) OVER (PARTITION BY m.platform ORDER BY m.date ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING)
                            ELSE 0 
                        END as conversions_zscore,
                        CASE 
                            WHEN SUM(m.clicks) OVER (PARTITION BY m.platform ORDER BY m.date ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING) > 0
                            THEN (m.clicks - AVG(m.clicks) OVER (PARTITION BY m.platform ORDER BY m.date ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING)) 
                                 / STDDEV(m.clicks) OVER (PARTITION BY m.platform ORDER BY m.date ROWS BETWEEN 7 PRECEDING AND 1 PRECEDING)
                            ELSE 0 
                ...
                anomaly_detection AS (
                    SELECT *,
                        CASE 
                            WHEN ABS(spend_zscore) > 2 OR ABS(revenue_zscore) > 2 OR ABS(conversions_zscore) > 2 OR ABS(clicks_zscore) > 2
                            THEN CASE 
                                WHEN ABS(spend_zscore) > 3 OR ABS(revenue_zscore) > 3 OR ABS(conversions_zscore) > 3 OR ABS(clicks_zscore) > 3
                                THEN 'high'
                                ELSE 'medium'
                            END
                            ELSE 'low'
                        ...
                        CASE 
                            WHEN ABS(spend_zscore) > 2 THEN 'Spend anomaly: ' || 
                                CASE WHEN spend_zscore > 0 THEN 'Unusual increase' ELSE 'Unusual decrease' END
                            WHEN ABS(revenue_zscore) > 2 THEN 'Revenue anomaly: ' || 
                                CASE WHEN revenue_zscore > 0 THEN 'Unusual increase' ELSE 'Unusual decrease' END
                            WHEN ABS(conversions_zscore) > 2 THEN 'Conversions anomaly: ' || 
                                CASE WHEN conversions_zscore > 0 THEN 'Unusual increase' ELSE 'Unusual decrease' END
                            WHEN ABS(clicks_zscore) > 2 THEN 'Clicks anomaly: ' || 
                                CASE WHEN clicks_zscore > 0 THEN 'Unusual increase' ELSE 'Unusual decrease' END
                ...
                SELECT 
                    ...
                    clicks,
                    conversions,
                    ROUND(spend_zscore, 2) as spend_zscore,
                    ROUND(revenue_zscore, 2) as revenue_zscore,
                    ROUND(conversions_zscore, 2) as conversions_zscore,
                    ROUND(clicks_zscore, 2) as clicks_zscore,
                    severity_level,
                    anomaly_description,
                    CASE 
                        WHEN spend_zscore > 2 OR revenue_zscore > 2 OR conversions_zscore > 2 OR clicks_zscore > 2 THEN 'positive'
                        WHEN spend_zscore < -2 OR revenue_zscore < -2 OR conversions_zscore < -2 OR clicks_zscore < -2 THEN 'negative'
                        ELSE 'neutral'
                    END as anomaly_type
                FROM anomaly_detection
                WHERE severity_level = $${paramIndex}
                ORDER BY date DESC, ABS(GREATEST(spend_zscore, revenue_zscore, conversions_zscore, clicks_zscore)) DESC
                LIMIT $${paramIndex + 1} OFFSET $${paramIndex + 2}
            `;
            
            queryParams.push(severity, limit, offset);
            const result = await client.query(anomalyQuery, queryParams);
            ...
            const anomalies = result.rows.map(row => ({
                ...
                z_scores: {
                    spend: parseFloat(row.spend_zscore),
                    revenue: parseFloat(row.revenue_zscore),
                    conversions: parseFloat(row.conversions_zscore),
                    clicks: parseFloat(row.clicks_zscore)
                },
