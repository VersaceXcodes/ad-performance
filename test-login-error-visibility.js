...
async function testLoginErrorVisibility() {
  ...
  try {
    ...
    await page.waitForTimeout(2000); // Wait 2 seconds for error to display
    ...
    if (!errorFound) {
      ...
      for (const keyword of errorKeywords) {
        if (pageText.toLowerCase().includes(keyword)) {
          console.log(`⚠️  Found error keyword "${keyword}" in page content, but no specific error element detected`);
          errorFound = true;
          break;
        }
      }
    ...
    await page.waitForTimeout(1000); // Wait a bit more to catch any delayed responses
